...
public class General extends DatosDelega implements Serializable {
  ...
  public void doBuscarEmpleados() {
    try {
      ...
      this.attrs.put("idPersona", JsfBase.getAutentifica().getPersona().getIdUsuario());
  ...
  private void generaInforme() {
    ...
    try {
      //informe= new Informe(this.persona, this.attrs.get("login").toString(), this.attrs.get("contrasenia").toString(), JsfBase.getAutentifica().getPersona().getDescripcionPerfil());			
  ...
  public String doCancelar() {
    ...
    try {
      regresar = "/Paginas/Usuarios/Delegar/filtro.jom".concat(Constantes.REDIRECIONAR);
    } // try
    catch (Exception e) {
      JsfBase.addMessageError(e);
      Error.mensaje(e);
